...
if (focused) {
    ...
    draw_roundrect_ext(x -180 - border_thickness, y - 48 - border_thickness,
                       x + w + border_thickness, y + h + border_thickness,
                       42, 42, false);

    // Retângulo menor (fundo real)
    draw_set_color(c_white); // Cor de fundo
    draw_roundrect_ext(x - 180, y - 49, x + w, y + h, 35, 35, false);
...
if (string_length(text) > 0 || focused) {
    ...
    draw_text(x - 140 , y - 22, string(text));
} else {
	draw_set_color(make_color_rgb(174, 136, 236));
    draw_text(x - 140 , y - 22, string(placeholder));
}
